...
function StepBlock({ step }: { step: Step }) {
  return (
    ...
    >
      ...
      <div
        className={` md:hidden flex gap-x-2 md:gap-x-[10px] max-md:order-1  items-center text-sm`}
      >
        <div className="h-8 w-8 flex items-center justify-center bg-white border-[.4px] border-[#D9D9D9] rounded-full text-center">
          {step.id + 1}
        </div>
        <div>Step</div>
      </div>
      ...
      <div className="text-ash font-medium max-w-[600px] flex-1 ">
        <HighlightedText
          ...
          className="text-xl md:text-2xl font-AgrandirBold mb-3 text-raisin-black"
          highlightClassName="font-AgrandirRegular"
        />
        <p className="text-sm sm:text-base mb-1">{step.subtext}</p>
        <ul className="list-disc ml-6 md:ml-[30px] text-sm sm:text-base leading-8">
